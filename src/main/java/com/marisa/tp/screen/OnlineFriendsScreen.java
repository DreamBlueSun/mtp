...
import com.marisa.tp.screen.btn.FriendsTpBtn;
...
public class OnlineFriendsScreen extends Screen {
    ...
    ResourceLocation resourceLocation = new ResourceLocation(MTp.MOD_ID, "textures/gui/friend/window.png");

    private final List<PlayerInfo> onlinePlayer;
    private List<PlayerInfo> onlineFriends;
    ...
    protected OnlineFriendsScreen(List<PlayerInfo> onlinePlayer) {
        super(new TextComponent("mtp.screen.online.friends"));
    ...
    private void update() {
        if (this.onlinePlayer != null && Minecraft.getInstance().player != null) {
            ...
            this.onlineFriends = this.onlinePlayer.stream().filter(i -> friendsUUID.contains(i.getUuid())).collect(Collectors.toList());
            this.pageMax = this.onlineFriends.size() % PAGE_SIZE == 0 ? Math.max(this.onlineFriends.size() / PAGE_SIZE - 1, 0) : this.onlineFriends.size() / PAGE_SIZE;
    ...
    public void render(@NotNull PoseStack matrixStack, int mouseX, int mouseY, float particleTick) {
        ...
        for (int i = PAGE_SIZE * this.page; i < this.onlineFriends.size(); i++) {
            drawString(matrixStack, this.font, this.onlineFriends.get(i).getName(), x, y + 6, 0x00FF7F);
            this.btnList.add(new FriendsTpBtn(x + 70, y, 32, 20, new TextComponent("传送"), this.onlineFriends.get(i).getUuid()));
            y += 22;
            if (++count == PAGE_SIZE) {
                break;
            }
    ...
    public static void open(List<PlayerInfo> onlinePlayer) {
